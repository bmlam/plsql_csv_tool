...
/**************************************************************/
	...
	BEGIN
		pck_std_log.dbx('l_line_len: '||l_line_len||' First 80 chars: '||substr(p_line, 1,80) );
		WHILE l_scan_pos < l_line_len LOOP
		 pck_std_log.dbx( 'ltab_column.count: '||ltab_column.count);
...
BEGIN
	...
	g_col_sep	:= p_col_sep; 
	i$reset_line_cur;
	pck_std_log.dbx( 'g_col_sep='||g_col_sep );
...
-- test: exec Pkg_utl_csv.insert2table_from_file( 'test.txt', 'CSV_UTIL_LOAD_DIR',  'no such table' );
	...
	END i$prepare_4_insert;

BEGIN -- insert2table_from_file 
	pck_std_log.inf( 'file:'||p_file||' p_directory:'||p_directory );
	g_col_sep  := p_col_sep;	
	pck_std_log.dbx( 'g_col_sep='||g_col_sep );

	if p_standalone_head_line is not null then
	  vtab_col_name := get_all_columns (p_standalone_head_line);
	  if vtab_col_name.count = 0 then
		RAISE_APPLICATION_ERROR( -20001, 'Header line is given but no column names found!' );
	  end if; -- check column names count
	...
	LOOP 
	 BEGIN 
		...
		v_ln_cnt := v_ln_cnt + 1;
		debug( c_cntxt, 'input line '||v_ln_cnt||' len:'|| lengthc( v_buf)||' start with: '||substrc( v_buf, 1, 80) );

	  -- special handling for null characters in file context
	...
	END IF;
	 ...
	 IF vtab_col_name.COUNT = 0 THEN -- column names still unknown 
		--
		-- preparation before INSERT 
		--
		  if length(v_buf) = 1 or v_buf is null  then
			 raise_application_error(-20000, 'the first line of the CSV text - assumed as header line - appears to be empty!');
		  end if; -- first line empty
...
BEGIN
	...
	FOR i IN 1 .. ptab_col_name.COUNT LOOP
		pck_std_log.dbx( i||'.'||'column: '|| ptab_col_name(i) );
		po_sql_text := po_sql_text || CASE
								  WHEN i > 1 THEN ', '
							  END || ':B' || TO_CHAR (i);
